...
func show_notification(title, text = ""):
	...
	if (scene_switch_root):
		...
		self.vrOrigin.get_parent().add_child(nw);
	var pos = self.vrCamera.global_transform.origin - self.vrCamera.global_transform.basis.z;
	
	nw.look_at_from_position(pos, self.vrCamera.global_transform.origin, Vector3(0,1,0));
